  ...
  const [servicesData, setServicesData] = useState([
    {
      ...
      marketPrice: "₹3,500",
      salesPrice: "₹2,999",
      unitOfMeasure: "Per session",
      minimumRequirement: "1 room minimum",
      minimumOrder: "1",
      additionalCostApplicable: "Yes",
      costName: "Extra stain removal",
      costType: "Fixed",
      value: "₹500",
      taxApplied: "Yes",
      hsnSacCode: "9984",
      ...
      discountName: "First-time customer",
      discountType: "Percentage",
      disValue: "10%",
      serviceBenefits: "Eco-friendly products, Satisfaction guarantee",
      requirement: "Provide access to all areas",
      policy: "24-hour cancellation policy",
      faqs: "What if I need to reschedule?",
    ...
    },
    {
      ...
      discountType: "",
      disValue: "",
      serviceBenefits: "90-day warranty on repairs",
      requirement: "Make AC accessible",
      policy: "Same-day service guarantee",
      faqs: "Is diagnosis free?",
    ...
    },
    {
      ...
      marketPrice: "₹50,000",
      salesPrice: "₹45,000",
      unitOfMeasure: "Per project",
      minimumRequirement: "Detailed requirements document",
      minimumOrder: "1",
      additionalCostApplicable: "Yes",
      costName: "Additional pages",
      costType: "Per unit",
      value: "₹2,000/page",
      taxApplied: "Yes",
      hsnSacCode: "9983",
      ...
      taxRate: "18%",
      discountApplicable: "Yes",
      discountName: "Bulk order",
      discountType: "Fixed",
      disValue: "₹5,000",
      serviceBenefits: "Free 1-year maintenance",
      requirement: "Content ready before start",
      policy: "3 revision limit",
      faqs: "What CMS do you use?",
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const formattedData = json.map((row) => ({
        ...
        additionalCostApplicable: row.additionalCostApplicable || "",
        ...
        serviceBenefits: row.serviceBenefits || "",
    ...
    switch (format) {
      ...
      case "pdf":
        ...
        filteredServices.forEach((service, index) => {
          const y = 20 + index * 10;
          doc.text(
            `${service.serviceCode}, ${service.serviceName}, ${service.serviceDescription}, ${service.serviceCategory}, ${service.serviceType}, ${service.marketPrice}, ${service.salesPrice}`,
            10,
            y
          );
        });
        ...
        const docxContent = filteredServices.map(
          (service) =>
            new Paragraph({
              children: [
                new TextRun({
                  text: `Service Code: ${service.serviceCode}, Name: ${service.serviceName}, Description: ${service.serviceDescription}, Price: ${service.salesPrice}`,
                }),
              ],
            })
        );
      ...
      <div className="controls-container">
        ...
        <div className="search-by-dropdown">
          <select value={searchBy} onChange={handleSearchBy}>
            ...
            <option value="hsnSacCode">HSN/SAC Code</option>
          </select>
        </div>
      ...
      <table className="full-width-table table-bordered table-striped">
        <thead>
          <tr>
            ...
            <th>Consumer Type</th>
            <th>Market Price (MP)</th>
            <th>Sales Price (SP)</th>
            <th>Unit of Measure</th>
            <th>Minimum Requirement</th>
            <th>Minimum Order (MOQ)</th>
            <th>Additional Cost Applicable?</th>
            <th>Cost Name</th>
            <th>Cost Type</th>
            <th>Value</th>
            <th>Tax Applied</th>
        ...
        <tbody>
          {filteredServices.map((service, index) => (
            <tr key={index}>
              ...
              <td>{service.consumerType}</td>
              <td>{service.marketPrice}</td>
              <td>{service.salesPrice}</td>
              <td>{service.unitOfMeasure}</td>
              <td>{service.minimumRequirement}</td>
              <td>{service.minimumOrder}</td>
              <td>{service.additionalCostApplicable}</td>
              <td>{service.costName}</td>
              <td>{service.costType}</td>
              <td>{service.value}</td>
              <td>{service.taxApplied}</td>
              <td>{service.hsnSacCode}</td>
              <td>{service.taxType}</td>
              <td>{service.taxRate}</td>
              <td>{service.discountApplicable}</td>
              <td>{service.discountName}</td>
              <td>{service.discountType}</td>
              <td>{service.disValue}</td>
              <td>{service.serviceBenefits}</td>
              <td>{service.requirement}</td>
              <td>{service.policy}</td>
              <td>{service.faqs}</td>
              <td>{service.serviceImage}</td>
              <td>{service.serviceVideo}</td>
